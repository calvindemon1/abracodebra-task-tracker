...
export default function DashboardHome() {
  ...
  // ===== FETCH DATA (Disesuaikan dengan struktur API lu) =====
  const [tasks] = createResource(async () => {
    const res = await TasksService.list();
    return res.data || []; // Pakai .data
  });

  const [users] = createResource(async () => {
    const res = await UsersService.list();
    return res || []; // Root array
  });
  ...
  const [works] = createResource(async () => {
    ...
    // Berdasarkan console lu, datanya di res.data (array of task objects with works array inside)
    return res.data || [];
  });
  ...
  const visualData = createMemo(() => {
    ...
    const allWorksData = works() || []; // Ini array yang isinya { task_id, works: [] }

    let result = [];

    // Helper Progress Task
    const getTaskProgress = (taskId) => {
      const workObj = allWorksData.find((w) => w.task_id === taskId);
      return workObj ? Math.round(workObj.progress || 0) : 0;
    };

    // Helper Workload Count (Jumlah total activity_name)
    const getTaskWorkloadCount = (taskId) => {
      const workObj = allWorksData.find((w) => w.task_id === taskId);
      return workObj ? workObj.works?.length || 0 : 0;
    };
    ...
    if (selectedTeamId() === "All") {
      ...
      result = allTeams.map((team) => {
        ...
        const totalPerformance = teamTasks.reduce(
          (acc, t) => acc + getTaskProgress(t.id),
          0,
        );
        const totalWorkload = teamTasks.reduce(
          (acc, t) => acc + getTaskWorkloadCount(t.id),
          0,
        );

        return {
          label: team.name, // API teams lu fieldnya 'name' bukan 'team_name'
          performance:
            teamTasks.length > 0
              ? Math.round(totalPerformance / teamTasks.length)
              : 0,
      ...
      result = membersInTeam
        .map((member) => {
          ...
          const totalPerformance = userTasks.reduce(
            (acc, t) => acc + getTaskProgress(t.id),
            0,
          );
          const totalWorkload = userTasks.reduce(
            (acc, t) => acc + getTaskWorkloadCount(t.id),
            0,
          );
          ...
          return {
            ...
            performance:
              userTasks.length > 0
                ? Math.round(totalPerformance / userTasks.length)
                : 0,
            workload: totalWorkload,
  ...
  const pieData = createMemo(() => {
    const allTasks = tasks() || [];
    const counts = { TODO: 0, IN_PROGRESS: 0, DONE: 0, ON_HOLD: 0 };

    allTasks.forEach((t) => {
      const s = t.status?.toUpperCase().replace(" ", "_") || "TODO";
    ...
    return {
      ...
      datasets: [
        {
          data: [counts.TODO, counts.IN_PROGRESS, counts.DONE, counts.ON_HOLD],
  ...
  const barData = () => ({
    ...
    datasets: [
      {
        label: "Performance (%)",
        data: visualData().map((d) => d.performance),
        backgroundColor: "rgba(99, 102, 241, 0.8)",
        borderRadius: 8,
        yAxisID: "y",
      },
      {
        label: "Workload (Activities)",
        data: visualData().map((d) => d.workload),
  ...
  return (
    <div class="p-6 space-y-8 min-h-screen text-white pb-20">
      ...
      >
        <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-8">
          <div>
            ...
            <p class="text-gray-500 text-[10px] font-black uppercase tracking-[0.4em] mt-2 italic text-indigo-400">
              Activity & Workload Monitoring
            </p>
          </div>
      ...
      <Show
        when={
          !tasks.loading && !users.loading && !teams.loading && !works.loading
        }
        fallback={
      ...
      >
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* BAR CHART PERFORMANCE VS WORKLOAD */}
          <div class="lg:col-span-2 bg-gray-900/40 backdrop-blur-3xl border border-white/10 p-10 rounded-[40px] shadow-2xl section-fade">
            <div class="flex items-center gap-4 mb-10">
              ...
              <div>
                ...
                <p class="text-[10px] text-indigo-400 font-bold uppercase tracking-widest italic">
                  Efficiency Index
                </p>
              </div>
            </div>
        ...
        >
          <div class="p-8 border-b border-white/5 bg-white/5 flex items-center justify-between">
            ...
            <span class="text-[9px] font-black bg-indigo-500/10 px-4 py-2 rounded-full text-indigo-400 border border-indigo-500/20 uppercase tracking-widest">
              {visualData().length} Units Tracked
            </span>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full text-left">
              <thead class="text-[9px] uppercase tracking-[0.3em] font-black text-gray-600 border-b border-white/5">
                <tr>
                  <th class="px-10 py-6 italic">Identity</th>
                  <th class="px-10 py-6 text-center italic">Tasks</th>
                  <th class="px-10 py-6 text-center italic">
                    Activities (Load)
                  </th>
                  <th class="px-10 py-6 italic">Performance Index</th>
                  <th class="px-10 py-6 text-right italic">Rank</th>
                </tr>
              </thead>
                  ...
                  {(item, i) => (
                    <tr class="hover:bg-white/[0.02] transition-all group">
                      <td class="px-10 py-8 flex items-center gap-4">
                        ...
                        <div class="font-black text-white group-hover:text-indigo-400 transition-colors uppercase tracking-tight italic">
                          {item.label}
                        </div>
                      </td>
                      <td class="px-10 py-8 text-center font-mono font-bold text-xs text-blue-400">
                        {item.taskCount}
                      </td>
                      <td class="px-10 py-8 text-center font-mono font-bold text-xs text-pink-400">
                        {item.workload}
                      </td>
                      <td class="px-10 py-8">
                      ...
                      <td class="px-10 py-8 text-right font-black text-gray-600 italic">
                        #{i() + 1}
                      </td>
                    </tr>
                  )}
